function [X_endurence, y_endurence] = augment_and_process_endurence_dataset_4_CNN(database, ...
    new_spall_orders, new_d_order, num_harmonies, num_of_first_indices_2_set_2_zero)
...
max_ind = ceil(max(new_spall_orders) * num_harmonies / new_d_order) ;
X_endurence = zeros(max_ind, 3 * num_sigs) ; % pre-allocation
...
while first_ind <= num_sigs
    ...
    [sigs_order_env, y_endurence_batch] = augment_endurence_signals(sigs_env_order, ...
        database.spall_orders(1), new_spall_orders, database.d_order, new_d_order) ;
    
    X_endurence_batch = sigs_order_env ;
    X_endurence_batch(1:num_of_first_indices_2_set_2_zero, :) = ...
        zeros(num_of_first_indices_2_set_2_zero, size(X_endurence_batch, 2)) ;
    X_endurence_batch = X_endurence_batch(1:max_ind, :) ;
